...
for filename in glob.iglob('bachelor-thesis-info-mirror/artifacts/benchs/*.json'):
	if not os.path.isfile(filename):
		continue
	for figsize, suffix in [([12, 7], ''), ([15, 6], '-wide')]:
		...
		for i, (lab, y) in enumerate(zip(labels, times)):
			...
			plt.scatter(x, y, alpha=(1 - 1/(1+np.exp(-len(x) / 100))), marker=pmark, color=pcol)
		...
		plt.savefig(f'build/{base}{suffix}.svg', format='svg', bbox_inches='tight')

		print("\ttotal")
		plt.figure(figsize=figsize)
		plt.bar(labels, totals, tick_label=labels)
		# plt.show()
		plt.savefig(f'build/{base}{suffix}.total.svg', format='svg')
