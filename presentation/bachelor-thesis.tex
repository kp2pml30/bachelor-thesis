...
\ifthenelse{\equal{\presMode}{full}}{
	...
	\author{студент: Прокопенко К.Д.\\руководитель: Беляев Е.А.\\консультант: Соломенников Д.И.}
...
\textbf{Цель:} Исследование возможностей и способов добавления взаимодействия языков с различными типизациями в среду исполнения на платформе Ark с минимизацией неуправляемого кода\\
\textbf{Задачи:}
\begin{itemize}
	...
	\item Реализация прототипа с минимизацией нативных прослоек
...
\begin{itemize}
	\item[\xmark] Нативный-нативный
		\begin{itemize}
			\item{C++, Zig, Carbon}
		\end{itemize}
	\pause
	\item[\xmark] Управляемый-нативный
		\begin{itemize}
...
\end{enumerate}
Однако, компоненты имеют обобщенные фрагменты; и задача разбивается на части:
\begin{enumerate}
	\item Изменения платформы
	\item Архитектуру межъязыкового взаимодействия
\end{enumerate}
\end{frame}
...
\begin{frame}
\frametitle{Чего не хватает для исполнения?}
\begin{center}
...
\end{tabular}
\end{center}
($\ast$)~--- заполняется при каждом вызове
