...
\begin{enumerate}
	\item ``Низкоуровневые'' изменения платформы
	\item Создание архитектуры межъязыкового взаимодействия
\end{enumerate}
\end{frame}
...
\begin{enumerate}
	...
	\item Обход стеков\only<2>{ \cmark}
	\begin{enumerate}
		\item определение типа метода
		\item выбор стеков из состояний потока\only<2>{ \cmark}
	\end{enumerate}
	\item Маркировка объектов\only<2>{ \cmark}
	\item Перемещение объектов\only<2>{ \cmark}
	\ifthenelse{\equal{\presMode}{full}}{
		\pause
